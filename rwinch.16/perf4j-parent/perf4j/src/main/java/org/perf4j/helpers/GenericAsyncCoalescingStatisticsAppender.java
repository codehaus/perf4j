...
public class GenericAsyncCoalescingStatisticsAppender {
     ...
     * 
     * @param handler
     *            The Perf4jEventHandler used to process GroupedTimingStatistics
     *            created by aggregating StopWatch log message. It also allows
     *            allows handling of StopWatches created by parsing the messages
     *            passed in.
     *            
    ...
    public void append(String message) {
        ...
        if (getStopWatchParser().isPotentiallyValid(message)) {
    ...
    private void stopDrainingThread() {
        try {
            ...
            getHandler().error("Unexpected error stopping AsyncCoalescingStatisticsAppender draining thread: "
                          + e.getMessage());
        }
    }
     ...
     * The Perf4jEventHandler is then called to deal with the created GroupedTimingStatistics.
     */
        ...
        public void run() {
            ...
            while (statsIterator.hasNext()) {
                try {
                    ...
                    getHandler().error("Error calling the GroupedTimingStatisticsHandler: " + e.getMessage());
                }
            }
        }
    }
    ...
    /**
     * This helper class pulls StopWatch log messages off the loggedMessages
     * queue and exposes them through the Iterator interface. For every non-null
     * StopWatch created the Perf4jEventHandler is then called to deal with the
     * created StopWatch.
     */
        ...
        private StopWatch getNext() {
            ...
            while (true) {
                ...
                while (!drainedMessages.isEmpty()) {
                    ...
                    StopWatch parsedStopWatch = getStopWatchParser().parseStopWatch(message);
